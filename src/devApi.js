...
const now = () => Date.now();
const DOCS = [];
function addDoc(name, text = "", extra = {}) {
    const words = typeof text === "string" && text.length
        ? (text.match(/\b\w+\b/g) || []).length
        : undefined;
    const pages = typeof words === "number" ? Math.max(1, Math.ceil(words / 400)) : undefined;
    const id = String(SEQ++);
    const d = {
        id,
...
const listDocs = () => DOCS.map(({ text, ...rest }) => ({
    ...rest, // includes id + docId internally, UI should use docId
}));
const getDoc = (id) => DOCS.find((d) => d.id === String(id ?? "") || d.docId === String(id ?? ""));
...
async function parseBodyJSON(init) {
    if (!init?.body)
        return {};
    try {
...
async function parseFormData(init) {
...
export function installDevApi() {
    ...
    if (window.__devApiInstalled)
        return;
    window.__devApiInstalled = true;
    ...
    window.fetch = async (input, init) => {
        ...
        if (url.includes("/api/find") && (!init || (init.method ?? "GET").toUpperCase() === "GET")) {
            ...
            if (!d)
                return jsonResponse({ ok: false, total: 0, matches: [] }, 404);
            if (!q)
                return jsonResponse({ ok: true, total: 0, matches: [] }, 200);
        ...
        if (url.includes("/api/audit") && (init?.method ?? "POST").toUpperCase() === "POST") {
            ...
            if (!d)
                return jsonResponse({ ok: false, findings: [], totalScore: 0, count: 0 }, 404);
            const base = [
        ...
        if (url.includes("/api/checklist/run") && (init?.method ?? "POST").toUpperCase() === "POST") {
            const j = await parseBodyJSON(init);
            const docIds = Array.isArray(j?.docIds) ? j.docIds : [];
