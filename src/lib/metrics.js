...
function monthlyInterestIO(loan, ratePct) {
    const r = ratePct / 100 / 12;
    return loan * r;
}
/** Full amortising repayment (monthly) at rate% for term years. */
function monthlyPaymentRepay(loan, ratePct, years) {
    const r = ratePct / 100 / 12;
...
export function computeMetricsWithRegion(values) {
    ...
    if (loan > 0) {
        if (product === "REPAY")
            monthlyDebtService = monthlyPaymentRepay(loan, ratePct, termY || 25);
        else
            monthlyDebtService = monthlyInterestIO(loan, ratePct);
    }
