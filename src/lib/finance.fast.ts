// src/lib/finance.fast.ts
export type Deal = {
    price: number;          // purchase price
    rentMonthly: number;    // rent per month
    loan: number;           // mortgage principal
    rate: number;           // annual nominal (e.g., 0.055)
    termYears: number;      // amortisation
    costsAnnual: number;    // operating costs (ins/maint/mgmt/etc.)
    product?: "IO" | "Repay";
  };
  
  export const toAnnual = (m: number) => m * 12;
  export const clampPos = (n: number) => (Number.isFinite(n) && n > 0 ? n : 0);
  ...
  export function calcSdltSimple(price: number, surcharge3pc = true) {
    ...
    if (surcharge3pc) tax += price * 0.03;
    return tax;
  }
  export function sdltImpactPct(price: number, sdlt: number) {
    return price > 0 ? sdlt / price : 0;
  }
  
  /** One-shot compute of all metrics for today’s screen. */
  export function computeMetrics(d: Deal) {
