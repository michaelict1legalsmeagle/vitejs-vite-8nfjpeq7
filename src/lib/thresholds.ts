 ...
 */
const REGION_FLOOR: Record<Region, number> = {
  England: 5.5,
  Wales: 5.5,
  Scotland: 6.0,
...
const LENDER_FLOOR: Record<string, number> = {
 ...
 */
export function stressFloorFor(
  region: Region,
  lender: string | null | undefined,
  _product: "" | "IO" | "REPAY" = ""
): number {
  const key = String(lender || "").trim().toLowerCase();
  if (key && key in LENDER_FLOOR) return LENDER_FLOOR[key];
