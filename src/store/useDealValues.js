...
function clampFloor(n, min) {
    if (n === null)
        return null;
    return Math.max(n, min);
}
function normalisePostcode(x) {
    if (typeof x !== "string")
        return "";
...
function parsePatch(current, patch) {
    const merged = { ...current, ...patch };
    const parsed = dealSchema.parse(merged);
    return toDealInputs(parsed);
}
export const useDealValues = create((set, get) => ({
    ...
    setValues(patch) {
        ...
        for (const [k, v] of Object.entries(delta)) {
            if (typeof v === "function") {
                ...
                resolved[k] = v(current);
            }
        }
        const next = parsePatch(current, resolved);
        set({ values: next });
    },
    reset() {
