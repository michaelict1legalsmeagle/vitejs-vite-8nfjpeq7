...
import DataExtractionSchema from "./schemas/dataExtraction";
// -------------------------
// Helpers
// -------------------------
...
function pickOfficialLotUrl(candidates, contextText) {
    ...
    for (const raw of all) {
        ...
        if (AGGREGATOR_HOST_SNIPPETS.some(s => host.includes(s)))
            score -= 5;
        if (KNOWN_AUCTION_HOST_SNIPPETS.some(s => host.includes(s)))
            score += 4;
        if (/[/-]lot[/-]?\d+/i.test(lower))
            score += 4;
        if (lower.includes("/lot/"))
            score += 3;
        if (lower.includes("/lots/"))
            score += 2;
...
function parseGuidePrice(context) {
    const text = context || "";
...
function boolPresence(haystack, needles) {
    const h = haystack.toLowerCase();
    return needles.some(n => h.includes(n.toLowerCase()));
}
export function extractListing(input) {
    ...
    let auction_house_name = input.lotMetadata?.auction_house_name ?? null;
...
export function tryExtractListing(input) {
    try {
...
function computeConfidence(params) {
    let c = 0.4;
    if (params.auction_house_url)
        c += 0.3;
    if (params.g.text)
        c += 0.2;
    if (params.g.lower !== null)
        c += 0.05;
    if (params.g.upper !== null)
        c += 0.05;
